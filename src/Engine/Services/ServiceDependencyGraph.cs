...
public sealed class ServiceDependencyGraph {
  ...
  public IEnumerable<Type> TopologicalSort() {
    ServiceDependencyGraph graph = new(this);
    Stack<Type> toVisit = new();

    // GD.Print("Initial roots: ");

    foreach (Type root in graph.GetRoots()) {
      // GD.Print($">> {root}");
    ...
    {
      int iterations = 0;
      int maxIterations = graph.Graph.Count;
      ...
      while (iterations < maxIterations) {
        while (toVisit.Any()) {
          ...
          IEnumerable<Type> dependents = graph.GetDependents(from);

          foreach (Type to in dependents) {
            graph.RemoveDependency(from, to);
          }

          graph.Graph.Remove(from);
        }
